...
_amb_copy_ssh_to_agent(){
    ...
    run_command _kubectl exec $ambari_server_name  sh -c "ssh-keyscan $host_name >> ~/.ssh/known_hosts"
    run_command _kubectl exec $ambari_server_name  sh -c "sshpass -p Zasd_1234 ssh-copy-id root@${host_name}"
}

config_master(){
    local ambari_server_name=$(_get_amb_server_name)
    run_command _kubectl exec $ambari_server_name  sh -c "echo -e  'y\n'|ssh-keygen -q -t rsa -N '' -f ~/.ssh/id_rsa"
...
_amb_start_agent_service() {
  ...
  run_command _kubectl exec $agent_name sh -c " echo Zasd_1234 | passwd root --stdin "
  run_command _kubectl exec $agent_name sh -c " systemctl restart ntpd "
}

amb_tool_get_server_sshkey() {
  _kubectl exec $(_get_amb_server_name)  sh -c "cat ~/.ssh/id_rsa"
}
